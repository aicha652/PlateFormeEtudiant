...
def signup(request):
	...
	if request.method == "POST":
		...
		try:
			...
			return render(request, "forum/signup.html", {"error": "l'email utilisé est déja existé."})
		except:
			if password1 == password2:
				...
				return render(request, "forum/signup.html", {"error": "password est incorrecte."})
...
def sign(request):
	...
	return render(request, "forum/completer_inscription.html")
...
def upvote(request, aid):
	...
	if len(upvotes) == 0:
		...
		mp.track(a.answer, 'nouvelle vote')
		u.save()
...
class CommentListView(ListView):
    model = Comment
    field_list = [
        'answer', 'author', 'datetime', 'comment'
    ]
...
class CommentDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['main_page_title'] = 'Comment Delete Confirmation'
        context['panel_name'] = 'Comments'
        context['panel_title'] = 'Delete Comment'
        return context
...
class SolutionsListView(ListView):
    model = Solutions
    field_list = [
        'answer', 'datetime', 'question', 'author', 'upvotes'
    ]


class SolutionsDeleteView(DeleteView):
    model = Solutions
    template_name_suffix = '_delete'
    success_url = reverse_lazy('solutions_list')


class QuestionListView(ListView):
    model = Question
    field_list = [
        'author', 'question', 'datetime', 'category'
    ]
...
class BlogCommentDeleteView(DeleteView):
    model = BlogPost
    template_name_suffix = '_delete'
    success_url = reverse_lazy('blogcomment_list')